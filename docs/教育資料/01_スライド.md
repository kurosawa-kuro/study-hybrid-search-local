...
## lexical と semantic をどう混ぜるか（設計選択）

ハイブリッド検索では「キーワード側の順位」と「意味側の順位」を最終順位に落とす混ぜ方が複数あります。

| 方式 | 仕組み | 長所 | 短所 |
|---|---|---|---|
| **特徴量化**（本 Phase の採用） | `me5_score` を LightGBM の 1 特徴量として渡し、融合をモデルに委ねる | 実装が単純。学習データから混ぜ方を学べる | スコアスケール差の吸収がモデル任せ。明示的な rank 情報を失う |
| **Rank Fusion**（Phase 3 で採用） | lexical rank と semantic rank を**順位ベース**で事前に融合してから rerank | スケール差に頑健。候補統合段が明確に独立 | 融合式のハイパラ調整が必要 |

### 代表的な Rank Fusion 式: RRF（Reciprocal Rank Fusion）

```
score(d) = Σ_i  1 / (k + rank_i(d))
```

- `rank_i(d)`: ランカー `i`（lexical / semantic）での文書 `d` の順位
- `k`: 定数（通常 60）で上位偏重を和らげる
- スコアの絶対値ではなく**順位だけ**を使うのでスケール差に強い

> 本 Phase は特徴量化で止めます。RRF は **Phase 3 で候補統合段として明示的に導入** されます。
...
## LightGBM の目的関数: LambdaRank

`src/trainers/lgbm_trainer.py`
...
### なぜ回帰 / 分類ではなく LambdaRank か

| 目的関数 | 最小化するもの | 学習の見方 |
|---|---|---|
| 回帰（Phase 1） | 予測値と正解値の**二乗誤差** | 1 件ずつの値を当てに行く |
| 分類 | クロスエントロピー | 1 件ずつのクラス確率を当てに行く |
| **LambdaRank**（本 Phase） | **NDCG を直接最大化**（ペアワイズの順序交換誤差） | 同一クエリ内の**順序関係**を学ぶ |

### ここで重要な 3 点
- `label=y` は**クリック / お気に入り / 問い合わせ**等の行動から作るラベル（gain 値）
- `group=group_sizes` で「同じ検索クエリの候補グループ」を LightGBM に教える → クエリ内でのみ順序を比較
- `metric=ndcg` + `ndcg_eval_at=[10]` で上位 10 件の NDCG を監視

> 「住宅価格を当てる」学習（Phase 1）と「検索結果を良い順に並べる」学習はまったく別タスクです。ランキング学習は **クエリ単位で順位だけ**を見に行きます。
...
2. ME5 で意味類似度を追加（`me5_score` を特徴量化）
3. LightGBM **LambdaRank** で事業指標込みの最終順位化

### 重要な見方
- 検索は 1 つのアルゴリズムで完結しない
- 「候補取得」と「順位最適化」を分けて考える
- ランキング学習は回帰 / 分類とは別タスク（クエリ内の順序を学ぶ）
- ログ設計まで含めて検索品質を改善する

---

## Phase 2 → Phase 3 で変わる点

次講義（GCP-ML 入門）へのブリッジ。**検索思想は変えずに実行基盤と融合方式を変える**のが Phase 3 の主題です。

| 軸 | Phase 2（本講義） | Phase 3（次講義） |
|---|---|---|
| lexical と semantic の融合 | `me5_score` を LightGBM 特徴量に入れる | **RRF で rank 融合**してから rerank |
| 候補抽出 semantic 側 | `property_embeddings` + Python cosine | **BigQuery `VECTOR_SEARCH`** |
| Lexical 検索 | Meilisearch on Docker | Meilisearch on **Cloud Run** |
| データストア | PostgreSQL | **BigQuery**（raw / feature_mart / mlops） |
| 特徴量生成 | バッチスクリプト | **Dataform** + assertions |
| キャッシュ | Redis | `cachetools.TTLCache`（in-memory） |
| ML モデル目的関数 | LightGBM **LambdaRank**（本講義で導入済） | 同（変わらず） |
| 実行環境 | Docker Compose | Cloud Run + Terraform + WIF |

### 引き継がれる設計（変わらないもの）
- 3 段構成（lexical 候補 → 意味類似度 → LightGBM rerank）
- 候補取得 / 再ランキングの責務分離
- LambdaRank + NDCG 監視
- フォールバック可能性（モデル未配置でも `/search` が返る）

> Phase 3 では「何を残し、何を置き換えるか」が主題です。設計パターンは本講義で固まっています。
